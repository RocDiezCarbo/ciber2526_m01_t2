...
AUDIO_FLIE_HASH=`echo "$AUDIO_FILE" | md5sum | cut -d " " -f 1`
...
echo "17. SEND. DATA_FILE_HASH"

DATA_FILE_HASH=`cat audio.wav | md5sum | cut -d " " -f 1`

echo "DATA_FILE_HASH $DATA_FILE_HASH | nc $IP_SERVER -q 0 $PORT"
